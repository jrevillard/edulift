...
test.describe('Family Lifecycle E2E', () => {
  ...
  test.describe('Resilience', () => {
    test('should handle API failure gracefully during family operations', async ({ page }) => {
      const authHelper = UniversalAuthHelper.forCurrentFile(page);
      const timestamp = Date.now();

      await test.step('Setup admin user on family manage page', async () => {
        await authHelper.setupAdminUser(
          'admin.resilience',
          `Resilience Admin ${timestamp}`,
          `Resilience Family ${timestamp}`
        );
      ...
      await test.step('Simulate API failure and verify page does not crash', async () => {
        // Intercept the next family API call and make it fail
        ...
        // App must still render the page (not crash or show blank screen)
        const pageTitle = page.locator('[data-testid="ManageFamilyPage-Heading-pageTitle"]');
      ...
      await test.step('Verify page recovers after API is restored', async () => {
        await page.reload();
        await page.waitForLoadState('networkidle');
